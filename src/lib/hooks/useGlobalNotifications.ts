...
export function useGlobalNotifications(user?: any) {
  ...
  const retryConnection = useRef((reason: string) => {
    ...
    if (!isVisibleRef.current && (reason === 'CHANNEL_ERROR' || reason === 'TIMED_OUT')) {
      console.log(`🌙 Page is hidden, deferring retry for: ${reason}`)
    ...
    setTimeout(() => {
      ...
      if (!user || !isVisibleRef.current) {
        console.log('🔕 Aborting retry - user unavailable or page hidden')
        return
      }
    ...
    initializeGlobalNotifications.current = async () => {
      try {
        ...
        if (!isOnlineRef.current) {
          console.log('📴 Offline - deferring notifications setup')
          reconnectOnVisibilityChange.current = true
          return
        }

        if (!isVisibleRef.current && hasBeenAFK.current) {
          console.log('🌙 Page hidden and AFK detected - deferring notifications setup')
          reconnectOnVisibilityChange.current = true
          return
        }
        ...
        if (!isOnlineRef.current) {
          console.log('📴 Connection lost during initialization, aborting')
          reconnectOnVisibilityChange.current = true
          return
        }
        ...
        const globalChannel = supabase
          ...
          .subscribe((status) => {
            console.log('🌐 Universal message listener status:', status)
            if (status === 'SUBSCRIBED') {
              ...
              // Avoid excessive retries during AFK periods
              if (!isVisibleRef.current) {
                console.log('🌙 Channel error during AFK - will retry when visible')
                reconnectOnVisibilityChange.current = true
              } else {
              ...
              // Check if this is an unexpected close vs intentional
              if (isVisibleRef.current && isOnlineRef.current && retryCountRef.current < maxRetries) {
                setTimeout(() => {
                  retryConnection.current('CLOSED')
                }, 2000) // Short delay for close events
              } else {
                console.log('🌙 Channel closed during AFK/offline or max retries reached')
                reconnectOnVisibilityChange.current = true
              }
            }
          })
